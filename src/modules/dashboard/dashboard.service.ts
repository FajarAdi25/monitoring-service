...
const SSL_CERTIFICATE_STATUS = ["VALID", "EXPIRING_SOON", "EXPIRED"] as const;
...
export class DashboardService {
    ...
    return {
      ...
      bySeverity: {
        [IncidentSeverity.CRITICAL]:
          result.bySeverity[IncidentSeverity.CRITICAL] ?? 0,
        [IncidentSeverity.MAJOR]:
          result.bySeverity[IncidentSeverity.MAJOR] ?? 0,
        [IncidentSeverity.WARNING]:
          result.bySeverity[IncidentSeverity.WARNING] ?? 0,
      },
      byType: {
        ...Object.fromEntries(NOMAD_INCIDENT_TYPES.map((type) => [type, 0])),
        ...Object.fromEntries(SSL_CERTIFICATE_STATUS.map((type) => [type, 0])),
        ...result.byType,
      },
    };
  }
  ...
  async recent(query: Record<string, unknown>) {
    ...
    return items.map((item) => {
      ...
      if (!metadata)
        throw new Error(
          `Cluster metadata missing for cluster ${item.clusterId}.`,
        );
