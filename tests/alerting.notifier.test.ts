...
async function sendAndReadPayload(kind: AlertNotificationKind, incident: IncidentEntity) {
  ...
  try {
    const notifier = new HttpWebhookAlertNotifier("http://telegram.test/webhooks/alerts");
